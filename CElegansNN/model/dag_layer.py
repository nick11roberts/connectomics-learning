...
class DAGLayer(nn.Module):
    def __init__(self, in_channel, out_channel, num_nodes, edges):
        ...
        self.weighted_adj = np.zeros([self.num_nodes, self.num_nodes])
        self.unweighted_adj = np.zeros([self.num_nodes, self.num_nodes])
        ...
        self.nodes = nn.ModuleList([
            NodeOp(in_degree=max(1, self.in_degree[x]),
                   ...
                   stride=2 if x in self.input_nodes else 1, node_num=x)
    ...
    def forward(self, y):
        ...
        while queue:
            ...
            node_num_list = self.rev_adjlist[now]
            # This is interesting -- there's one source and many sinks
            #if now in self.input_nodes:
            #    print(now)   
            #if now in self.output_nodes:
            #    print(now)
            # We don't want to capture the neuronal weights of the 
            # source vertices -- they aren't part of the graph
            if now not in self.input_nodes:
                weights = self.nodes[now].agg_weight.data.cpu().numpy()
                assert(len(node_num_list) == len(weights))
                for incoming, in_weight in zip(node_num_list, weights):
                    self.weighted_adj[incoming, now] = in_weight
                    self.unweighted_adj[incoming, now] = 1.0
        ...
        print(np.min(self.weighted_adj))
        print(np.max(self.weighted_adj))
        print(np.sum(self.weighted_adj))
        #weighted_adj_g = nx.from_numpy_matrix(weighted_adj)# + (unweighted_adj * 0.00000001))
        #print(nx.algorithms.smallworld.sigma(weighted_adj_g))
